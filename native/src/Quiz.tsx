...
export { questions, states, breeds, rankBreeds, personaFor };
...
export function BreedQuiz({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    ...
    questions.forEach(({ key }) => {
      ...
      const valid = key === "state" ? states.some((state) => state[1] === value) : questions.find((q) => q.key === key)?.options?.some((option) => option.value === value);
      if (value && valid) restored[key] = value;
    });
  ...
  const moveToTop = () => shellRef.current?.scrollIntoView({ block: "start", behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth" });
  ...
  const retake = () => {
    ...
    Object.values(answerParams).forEach((key) => url.searchParams.delete(key));
  ...
  return (
    <section ref={shellRef} aria-label="Dog breed matching quiz" className={`site-shell${embedded ? " is-embedded" : ""}`}>
      ...
      {view === "results" && (
        <section className="results-stage" aria-labelledby="results-title">
          <div className="results-heading">
            ...
            <h2 id="results-title">We found your pack.</h2>
            <p>
